...
function initSchema() {
  db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      id INTEGER PRIMARY KEY, username TEXT, first_name TEXT NOT NULL,
      last_name TEXT, photo_url TEXT, language_code TEXT DEFAULT 'en',
      is_banned INTEGER DEFAULT 0, is_admin INTEGER DEFAULT 0,
      referral_code TEXT UNIQUE NOT NULL, referred_by INTEGER,
    ...
    CREATE TABLE IF NOT EXISTS balances (
      ...
      stars_balance REAL DEFAULT 0, total_deposited REAL DEFAULT 0,
      total_withdrawn REAL DEFAULT 0, total_won REAL DEFAULT 0,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    );
    CREATE TABLE IF NOT EXISTS cases (
      id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT,
      image_url TEXT, price REAL NOT NULL DEFAULT 0,
      ...
      task_type TEXT DEFAULT 'none', task_value TEXT, task_min_referrals INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS case_rewards (
      ...
      reward_type TEXT NOT NULL, name TEXT NOT NULL, image_url TEXT,
      gift_emoji TEXT, stars_amount REAL DEFAULT 0, value REAL DEFAULT 0,
      rarity TEXT DEFAULT 'common', chance REAL NOT NULL, is_active INTEGER DEFAULT 1,
      FOREIGN KEY (case_id) REFERENCES cases(id) ON DELETE CASCADE
    );
    CREATE TABLE IF NOT EXISTS inventory (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      reward_id INTEGER NOT NULL, case_id INTEGER, status TEXT DEFAULT 'owned',
      obtained_at TEXT DEFAULT (datetime('now')), sold_at TEXT, stars_received REAL DEFAULT 0,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS inventory_history (
      ...
      user_id INTEGER NOT NULL, action TEXT NOT NULL, notes TEXT,
    ...
    CREATE TABLE IF NOT EXISTS withdrawals (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      inventory_id INTEGER NOT NULL, status TEXT DEFAULT 'pending',
      admin_id INTEGER, admin_notes TEXT,
      requested_at TEXT DEFAULT (datetime('now')), processed_at TEXT,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS deposits (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      method TEXT NOT NULL, amount REAL NOT NULL, stars_credited REAL DEFAULT 0,
      ton_tx_hash TEXT, ton_amount REAL, status TEXT DEFAULT 'pending',
      telegram_payment_charge_id TEXT, admin_id INTEGER, admin_notes TEXT,
    ...
    CREATE TABLE IF NOT EXISTS referrals (
      ...
      referred_id INTEGER NOT NULL, reward_given REAL DEFAULT 0,
      created_at TEXT DEFAULT (datetime('now')),
    ...
    CREATE TABLE IF NOT EXISTS upgrades (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      source_inventory_id INTEGER NOT NULL, target_reward_id INTEGER NOT NULL,
      source_value REAL NOT NULL, target_value REAL NOT NULL,
      win_chance REAL NOT NULL, result TEXT NOT NULL, result_inventory_id INTEGER,
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS settings (
      ...
      value TEXT NOT NULL, description TEXT, updated_at TEXT DEFAULT (datetime('now'))
    );
    ...
    CREATE TABLE IF NOT EXISTS broadcasts (
      id INTEGER PRIMARY KEY AUTOINCREMENT, admin_id INTEGER NOT NULL,
      message_text TEXT, image_url TEXT, button_text TEXT, button_url TEXT,
      target TEXT DEFAULT 'all', sent_count INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS case_opens (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      case_id INTEGER NOT NULL, reward_id INTEGER NOT NULL,
      inventory_id INTEGER, stars_spent REAL NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS daily_free_claims (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      case_id INTEGER NOT NULL, claimed_at TEXT DEFAULT (datetime('now')),
    ...
    CREATE TABLE IF NOT EXISTS promo_codes (
      ...
      case_id INTEGER NOT NULL, stars_required INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS promo_code_claims (
      id INTEGER PRIMARY KEY AUTOINCREMENT, promo_id INTEGER NOT NULL,
      user_id INTEGER NOT NULL, claimed_at TEXT DEFAULT (datetime('now')),
      expires_at TEXT NOT NULL,
  ...
  const insertSetting = db.prepare(`INSERT OR IGNORE INTO settings (key_name, value, description) VALUES (?, ?, ?)`);
  const defaults = [
    ...
    ['gift_withdrawal_min_stars', '10', 'Min stars required to withdraw gift'],
    ['stars_broadcast_amount', '5', 'Default stars amount for broadcast bonus'],
  ];
  for (const [k, v, d] of defaults) insertSetting.run(k, v, d);

  // Add is_demo column if missing (for existing DBs)
  try { db.exec(`ALTER TABLE case_opens ADD COLUMN is_demo INTEGER DEFAULT 0`); } catch {}
...
async function query(sql, params = []) {
  ...
  if (/^\s*(select|pragma)/i.test(normalized)) return stmt.all(...flatParams(params));
...
async function transaction(callback) {
  const fakeConn = {
    execute: async (sql, params) => { const res = await query(sql, params); return [res]; },
    all: async (sql, params) => { return await query(sql, params); },
  };
  db.exec('BEGIN');
  try {
