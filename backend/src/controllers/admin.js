...
async function getDashboard(req, res) {
  try {
    const [
      ...
      query(`SELECT u.username, u.first_name, co.created_at, c.name as case_name 
             FROM case_opens co JOIN users u ON u.id = co.user_id JOIN cases c ON c.id = co.case_id 
...
async function getUsers(req, res) {
  try {
    ...
    const users = await query(
      `SELECT u.*, b.stars_balance, b.total_deposited
       FROM users u
...
async function getUserProfile(req, res) {
  try {
    ...
    const user = await queryOne(
      `SELECT u.*, b.stars_balance, b.total_deposited, b.total_withdrawn, b.total_won
    ...
    const inventory = await query(
      `SELECT i.*, cr.name, cr.image_url, cr.value, cr.rarity, cr.reward_type
       FROM inventory i JOIN case_rewards cr ON cr.id = i.reward_id
       WHERE i.user_id = ? ORDER BY i.obtained_at DESC LIMIT 20`, [id]
    );
...
async function getCases(req, res) {
  try {
    const cases = await query(
      `SELECT c.*, COUNT(cr.id) as reward_count 
       FROM cases c LEFT JOIN case_rewards cr ON cr.case_id = c.id
...
async function createCase(req, res) {
  try {
    ...
    const [result] = await query(
      `INSERT INTO cases (name, description, image_url, price, case_type, referrals_required, task_type, task_value, task_min_referrals, win_chance, sort_order)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description || '', image_url || '', price, case_type || 'normal', referrals_required || 0, task_type || 'none', task_value || '', task_min_referrals || 0, win_chance || 50, sort_order || 0]
    );
...
async function updateCase(req, res) {
  try {
    ...
    await query(
      ...
      [name, description, image_url, price, case_type, is_active ? 1 : 0, referrals_required || 0, task_type || 'none', task_value || '', task_min_referrals || 0, win_chance || 50, sort_order || 0, id]
    );
...
async function getCaseRewards(req, res) {
  try {
    ...
    const rewards = await query(
      `SELECT * FROM case_rewards WHERE case_id = ? ORDER BY rarity DESC, chance ASC`, [case_id]
    );
    res.json({ rewards });
  } catch (err) {
...
async function createReward(req, res) {
  try {
    ...
    if (!case_id || !reward_type || !name || !chance) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    const [result] = await query(
      `INSERT INTO case_rewards (case_id, reward_type, name, image_url, gift_emoji, stars_amount, value, rarity, chance)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [case_id, reward_type, name, image_url || '', gift_emoji || '', stars_amount || 0, value || 0, rarity || 'common', chance]
    );
...
async function updateReward(req, res) {
  try {
    ...
    await query(
      ...
      [reward_type, name, image_url || '', gift_emoji || '', stars_amount || 0, value || 0, rarity || 'common', chance, is_active ? 1 : 0, id]
    );
...
async function getWithdrawals(req, res) {
  try {
    ...
    const withdrawals = await query(
      `SELECT w.*, u.username, u.first_name, cr.name as item_name, cr.value, cr.reward_type, cr.image_url, cr.gift_emoji
       FROM withdrawals w
...
async function approveWithdrawal(req, res) {
  try {
    ...
    await transaction(async (conn) => {
      ...
      await conn.execute(
        `UPDATE inventory SET status = 'withdrawn' WHERE id = ?`,
        [withdrawal.inventory_id]
      );

      await conn.execute(
        `INSERT INTO inventory_history (inventory_id, user_id, action, notes) 
         SELECT ?, user_id, 'withdrawn', 'Withdrawal approved by admin' FROM withdrawals WHERE id = ?`,
    ...
    const withdrawal = await queryOne(
      `SELECT w.*, u.id as uid FROM withdrawals w JOIN users u ON u.id = w.user_id WHERE w.id = ?`, [id]
    );
...
async function rejectWithdrawal(req, res) {
  try {
    ...
    await transaction(async (conn) => {
      ...
      const withdrawal = await queryOne(`SELECT inventory_id FROM withdrawals WHERE id = ?`, [id]);
      await conn.execute(
        `UPDATE inventory SET status = 'owned' WHERE id = ?`,
        [withdrawal.inventory_id]
      );
    });
...
async function approveDeposit(req, res) {
  try {
    ...
    const starsToCredit = custom_stars && parseFloat(custom_stars) > 0
      ? parseFloat(custom_stars)
      : parseFloat(deposit.stars_credited);
    ...
    await transaction(async (conn) => {
      ...
      await conn.execute(
        `INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, notes)
         VALUES (?, 'deposit', ?, ?, ?, 'TON deposit approved')`,
...
async function rejectDeposit(req, res) {
  try {
    const { id } = req.params;
    await query(
      `UPDATE deposits SET status = 'rejected', admin_id = ?, processed_at = NOW() WHERE id = ?`,
      [req.user.id, id]
    );
...
async function sendBroadcast(req, res) {
  try {
    ...
    const [result] = await query(
      `INSERT INTO broadcasts (admin_id, message_text, image_url, button_text, button_url, status)
       VALUES (?, ?, ?, ?, ?, 'sending')`,
...
async function updateSettings(req, res) {
  try {
    ...
    for (const [key, value] of Object.entries(updates)) {
      await query(
        `UPDATE settings SET value = ? WHERE key_name = ?`, [String(value), key]
      );
    }
...
async function getLogs(req, res) {
  try {
    ...
    if (type === 'admin') {
      rows = await query(
        `SELECT al.*, u.username, u.first_name FROM admin_logs al
      ...
      rows = await query(
        `SELECT co.*, u.username, u.first_name, c.name as case_name, cr.name as reward_name
         FROM case_opens co
      ...
      rows = await query(
        `SELECT upg.*, u.username, u.first_name
         FROM upgrades upg
         JOIN users u ON u.id = upg.user_id
         ORDER BY upg.created_at DESC LIMIT ? OFFSET ?`,
        [parseInt(limit), offset]
      );
    }
...
async function adjustBalance(req, res) {
  try {
    ...
    await transaction(async (conn) => {
      ...
      await conn.execute(
        `UPDATE balances SET stars_balance = ? WHERE user_id = ?`, [balAfter, id]
      );
...
module.exports = {
  ...
  sendBroadcast, getSettings, updateSettings, getLogs,
};
